...
buplishable_key ='pk_test_51ITokaB2sW6f5ETwftSKUDbFnoo2XyQhWmPxFWianevkNu2APWoGdW1DABa2cs8OzmKH3FSYyhxdmXSeqttj5gsE00zY2cxjtd'
# stripe.api_key ='sk_test_9JlhVB6qwjcRdYzizjdwgIo0Dt00N55uxbWy'
stripe.api_key ='sk_test_51ITokaB2sW6f5ETw2JSzFlIapq5ZyBiHyTzTa4xjEwC7tWlMHr5ZNfoFKYNaiK7z8ADQ61A5BWvixuRUaPZG7cYb00dqUFRsRH'
...
def customer_register():
    ...
    if form.validate_on_submit():
        ...
        flash(f'Đăng ký tài khoản {form.name.data} thành công', 'success')
        db.session.commit()
...
def customerLogin():
    ...
    if form.validate_on_submit():
        ...
        flash(f'Sai Emai hoặc mật khẩu','danger')
        return redirect(url_for('customerLogin'))
...
def get_order():
    if current_user.is_authenticated:
        ...
        try:
            ...
            flash('Chúng tôi đã nhận đơn hàng của bạn','success')
            return redirect(url_for('orders',invoice=invoice))
        except Exception as e:
            print(e)
            flash('Giao dịch đã xảy ra sự cố', 'danger')
...
def orders(invoice):
    if current_user.is_authenticated:
        ...
        for _key, product in orders.orders.items():
            ...
            subTotal -= discount
            tax = ("%.2f" % (.1 * float(subTotal)))
            grandTotal = ("%.2f" % (1.1 * float(subTotal)))
...
@app.route('/showcustomerorders')
def showcustomerorders():
    customer_id = current_user.id
    customerorders = CustomerOrder.query.filter_by(customer_id=customer_id).order_by(CustomerOrder.id.desc()).all()
    return render_template('customer/show_customer_order.html', title='customerorders',customerorders=customerorders)
